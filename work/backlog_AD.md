...
(cả bốn nay đã đóng — `U-041` là câu cuối, đóng 2026-09-08).

⚠️ **Ngày 2026-09-06, chủ quán đóng thêm `U-034` · `U-038` · `U-039` · `U-040`** (lúc ấy chỉ
`U-041` còn mở). ⇒ **ADM-03 hết chặn** (U-039 · U-040 cả hai đã có lời) → chuyển **loại 2**;
**ADM-10 · ADM-12 · ADM-13** hẹp bớt một vế chặn (`U-034` đóng, nhưng các câu `B11`…`B21` của chính
nhánh B vẫn chưa có lời — ba việc **vẫn** loại 1).

⚠️ **Ngày 2026-09-08, chủ quán đóng `U-041`** — vế *"còn thiếu gì không"* của mục tổng quan là
thiếu **NGUYÊN LIỆU**, tập nguyên liệu là *Danh mục nguyên liệu* `master_plan/shop-facts.md` §8.4
(T-067). **ADM-04 KHÔNG đổi loại:** lời ấy chốt *thiếu cái gì* nhưng không chốt *máy biết bằng cách
nào* ⇒ mở **U-045**, nên ADM-04 **vẫn loại 1**, nay chặn bởi `U-045` · `F52` · `F53`. Con số ba
loại ở bảng dưới **không đổi** (20 · 8 · 1).

⚠️ **Hai lượt sau, cùng ngày, vế ấy hoá ra có CẢ BA đường** — thêm **món** (tập ở `shop-facts.md`
§4.9, T-068) và **người** (tập ở **bảng phân vai §3**, T-076). **ADM-04 vẫn loại 1** và vẫn đúng ba
vế chặn cũ — nhưng đường *người* kéo **ADM-21 vào cùng một màn**: vế *thiếu người* của mục tổng
quan đứng trên `C36` (*ai đang trực trạm nào*), và trên **`U-049`** (*người đi giao là một trong
bốn vai của §3 hay người thứ năm*). ⇒ **ADM-04 và ADM-21 không còn tách rời được**: làm xong ADM-04
mà `C36` chưa có lời thì hàng 7 của §8.6 hiện được hai phần ba, thiếu đúng phần *người*. Con số ba
loại **không đổi** (20 · 8 · 1) — `U-049` không đẻ ra việc mới, nó làm rộng vế chặn của việc đã có.

⚠️ **Cùng ngày 2026-09-08, lượt kế tiếp, chủ quán ĐÓNG `U-049`** (T-071): người đi giao là **một
trong bốn vai** của `master_plan/shop-facts.md` §3, **POS chỉ định** từng lần — nguyên văn *"1
trong bốn vai trên có thể là bất cứ ai pos sẽ chỉ định."* ⇒ **con số nhân sự thật của nhánh C đóng
lại: bốn vai + chủ quán**, không có dòng thứ năm nào cho việc đi giao, nên `C23` nay chỉ còn hỏi
đầu người. **Nhưng lời ấy mở `U-050`** — *lúc một vai rời quán đi giao thì trạm của người ấy ai
...
✅ **Ngày 2026-09-16, chủ quán đóng `U-052`** (T-078) — nguyên văn *"người đứng quầy khônng đi
giao"* ⇒ chữ *"bất cứ ai"* của `U-049` **hẹp lại còn BA vai**; trạm `quay` không bao giờ trống vì đi
giao, nên **ADM-21** hết chờ một câu U-XXX nào cho việc xếp ca — nó chỉ còn đứng trên `C36`
...
(`work/backlog.md`). ⇒ **ADM-20 · ADM-22 · ADM-23 hết vế chặn `Đ-4`** — chúng vẫn loại 1, nhưng nay
chỉ còn chặn bởi các câu `C23`…`C33` của chính chúng.
...
- **ADM-04 vẫn loại 1**, vế chặn **đổi chủ chứ không mất**: `U-045` ⇒ `U-051` ⇒ **`U-054`**.
  `U-051` (*mục tổng quan bày GÌ ở vế nguyên liệu*) **đóng 2026-09-16** — bày **thời gian nhập** ·
  **tổng đã dùng** · **số thiếu máy tự trừ** (*thiếu = tổng đã nhập − tổng đã dùng*) — và chính lời
  đáp ấy mở **`U-054`**: hai con số *tổng* cộng dồn **từ mốc nào**. Cộng `F52` · `F53` như cũ.
- **Cùng lời ấy trả lời luôn câu `B21`** của nhánh B — câu hỏi *máy nhắc dựa vào cái gì*. ⇒
  **ADM-13** mất nửa *ngưỡng + lời nhắc* (chủ quán bác: máy không nhắc), nửa còn lại — **con số
  tồn ước tính** — đứng nguyên và nay đứng trên **`U-054`** (qua `U-051`, đóng 2026-09-16) chứ
  không trên `B21`; **ADM-14** hẹp lại
...
`U-034`·`U-038`·`U-039`·`U-040` cùng ngày), hai mươi chín việc chia làm **ba loại**, và loại quyết
định ai nhận nó:
...
## Hai lời từng treo — **cả hai đã về owner 2026-09-20 (ADM-53)**

| Mã | Nội dung | Nay đọc ở |
|---|---|---|
| **Đ-2** | thứ tự làm | `work/backlog.md` → *Thứ tự làm giữa lane admin và các pha*. Vế cũ *đóng nốt chuỗi BA trước* **đã xong**; vế mới 2026-09-20: lane admin chạy **song song** pha 2, nhưng chỉ ở nghĩa **thu luật** — xem mục ngay dưới |
| **Đ-4** | mảng con người làm tới **cả ba mức**: trực trạm + chấm công + lương | `master_plan/shop-facts.md` **§8.7** · `docs/product/0-ba/admin/01-ranh-gioi.md` §1.6 · `docs/product/1-system-design/architecture.md` **§14.4** |

**Đừng đọc hai lời ấy từ mục này** — nó chỉ trỏ (`work/findings.md` **F-001**). Cửa mà cả bốn lời
chốt ngày 2026-09-01 phải đi qua là *chủ quán xác nhận lại → chuyển về owner*; cả bốn đã đi hết, và
`work/admin-questions.md` §1 nay co lại còn một bảng lịch sử.

~~**U-034**~~ — mục tổng nhập hàng ngày ghi **con số gì** — **đóng 2026-09-06**: HAI con số, mua
vào và đã dùng (`master_plan/shop-facts.md` §8.4). Bốn việc nhánh B (**ADM-10**…**ADM-13**) vẫn
chặn bởi các câu `B11`…`B21` của chính nhánh B, chưa hết chặn.

## Lane này xếp lịch thế nào so với các pha — **có lời 2026-09-20**

**Owner của lời này là `work/backlog.md`** (mục *Thứ tự làm giữa lane admin và các pha*), vì nó là
dữ kiện xếp lịch của **repo**, không phải dữ kiện của quán. Mục này chỉ trỏ.

Chủ quán chốt 2026-09-20 (**Đ-2**, vế mới, hỏi ở ADM-53): lane admin chạy **SONG SONG** với pha 2 —
*hỏi chủ quán về admin trong khi pha 2 chạy*. Hai nửa của lời ấy, đọc đủ ở owner:

- ✅ **được làm song song: THU LUẬT** — đem các câu `A`…`F` còn mở ở `work/admin-questions.md` §3
  đi hỏi, và chuyển lời về owner. Hai mươi việc *loại 1* của bảng trên vì thế **hết phải chờ pha
  2 đóng**.
- ⛔ **không làm song song: THI CÔNG** — `docs/decisions.md` **ADR-031** vẫn đứng: ba mảng đi
  **sau** mảng bán hàng. Chữ *sau* ở ADR-031 nói về thi công; lời 2026-09-20 nói về thu luật. Đừng
  đọc cái này thành phép dựng phần admin trước.
...
`work/admin-questions.md` §3, `ADM-XX` trỏ xuống entry trong chính
...
**Còn thiếu gì — và chỗ này KHÔNG thuộc lane này:**
- ~~**U-038**~~ — con số nhập vào máy là một tổng hay một bảng theo mệnh giá — **đóng 2026-09-06:
  CẢ HAI.** Không đổi luật ở §8.5 hay I-021; đổi **cách chứng minh** I-021 (tổng cộng dùng cho phép
  trừ, bảng mệnh giá là cách đếm/kiểm). Ghi ở `master_plan/shop-facts.md` §8.5.
- **Ai gõ con số ấy vào máy và gõ lúc nào.** Chủ quán nói *ai bỏ tiền vào két* (chính chủ quán),
  không nói *ai gõ*. Không suy hộ (`CLAUDE.md` §3.5) — vẫn chưa có lời.
...
**L1** · nhánh A · **loại 2 — luật ĐÃ ĐỦ, thi công thuộc pha 2–4** (đóng 2026-09-06: `U-039` và `U-040` cả hai đã có lời)
**Prompt:** chưa viết được (luật 6 đầu file)
...
| **Bàn đã đánh số sẵn chưa** | **Đã đánh số CẢ MƯỜI LĂM** — 11 bàn ban đầu chốt 2026-09-06 (U-040), bốn bàn mới đánh số **nối tiếp 12–15** chốt **2026-09-16** (vế cuối của **U-042**, nay đóng) ⇒ danh sách bàn là **1…15**, vế **ADR-027** cần (*gọi tên được từng bàn*) **hết hở** |
...
- **ADR-027 kiểm chứng được cho CẢ MƯỜI LĂM bàn** — danh sách bàn gọi tên được là **1…15**: 11 bàn
  cũ đã đánh số sẵn (U-040, 2026-09-06), bốn bàn mới đánh số **nối tiếp 12 · 13 · 14 · 15** (chủ
  quán chốt **2026-09-16**, đóng nốt **U-042**). Sức chứa cũng hết hở: 4 chỗ/bàn cho cả mười lăm
  (2026-09-08). ⇒ việc này **không còn câu hỏi nào của chủ quán đứng trước nó**.
...
(bàn **1…15**, bốn bàn mới là 12–15 — **U-042** đóng 2026-09-16; chỗ ngồi thì cả 15 bàn đều 4).
...
**Đã có lời, và đã về owner** — `master_plan/shop-facts.md` **§8.6** giữ **bảy con số** chủ quán
gọi tên: đang làm cho bao nhiêu người ăn · đưa ra được bao nhiêu · còn thiếu bao nhiêu · bán được
như nào rồi · **số tiền dự tính** đã bán được · có bao nhiêu người đang làm · **còn thiếu gì không**
...
`B11` (kể tên thứ quán mua vào) · `B12` (mỗi thứ đơn vị gì). ~~`U-034`~~ **đóng 2026-09-06**: mục
tổng ghi HAI con số — mua vào và đã dùng (`master_plan/shop-facts.md` §8.4) — nên danh mục vẫn cần
đủ **hai** con số ấy cho mỗi dòng, nhưng loại con số không còn là câu hỏi mở.
...
- **ADM-13 (tồn ước tính) có vế trừ rồi, nhưng chưa có danh mục để chạy nó.** `U-034` đóng cho biết
  *đã dùng* là số nhập tay, không phải số máy suy ra — nên tồn ước tính = cộng dồn (mua − dùng) là
...
✅ **Lớp chặn THỨ HAI của cả nhánh C đã gỡ, 2026-09-20 (ADM-53).** Lớp ấy là lời chốt **Đ-4** —
*mảng con người làm cả ba mức* — từng chỉ sống trong `work/admin-questions.md` §1, một file tự khai
không sở hữu sự thật nào và sẽ bị xoá. Nay nó ở owner: `master_plan/shop-facts.md` **§8.7** ·
`docs/product/0-ba/admin/01-ranh-gioi.md` §1.6 · `docs/product/1-system-design/architecture.md`
**§14.4**.
...
**L1** · nhánh F · **loại 3** · ✅ **XONG NGÀY 2026-09-20** — nhánh **B** của *Acceptance* (chủ quán
trả lời được trong lượt)
**Prompt:** [`prompt/AD/ADM-53-hai-loi-ve-owner-L1.md`](../prompt/AD/ADM-53-hai-loi-ve-owner-L1.md) — viết 2026-09-04 (T-058)

**Xong ngày 2026-09-20 — ba câu hỏi, ba lời.** Chủ quán trả lời cả ba trong một lượt:

| Câu | Lời chủ quán | Đi đâu |
|---|---|---|
...
đúng như thế ở owner. Đây là lần thứ n của họ lỗi *"con số/mốc viết trong prompt đã hết hạn"*
(`work/findings.md` **F-003** · **F-018**) — đọc mốc ở owner, đừng đọc ở entry này.

⚠️ **Mục §8 của `shop-facts.md` nhận số §8.7, không phải §8.5 như entry viết bên dưới.** §8.5 và
§8.6 đã bị hai lượt khác lấy trong ngày 2026-09-04; số thật lấy theo §8.3 tại thời điểm chạy.
...
| **Đ-4** — mảng con người làm **cả ba mức** | ✅ 2026-09-20 (ADM-53) | `shop-facts.md` **§8.7** · `01-ranh-gioi.md` §1.6 · `architecture.md` **§14.4** |
